<?php
include_once("/var/www/classes/system.config.php");
...
<head>
	<meta charset="utf-8" />
	<title><?=COMPANY_NAME?> USA | Login</title>
	...
	<link href="<?=BASE_PATH?>assets/plugins/bootstrap/css/bootstrap-responsive.min.css" rel="stylesheet" type="text/css"/>
	<link href="<?=BASE_PATH?>assets/plugins/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css"/>
	<link href="<?=BASE_PATH?>assets/css/style-metro.css" rel="stylesheet" type="text/css"/>
	...
	<link rel="stylesheet" type="text/css" href="<?=BASE_PATH?>assets/plugins/select2/select2_metro.css" />
	<!-- END GLOBAL MANDATORY STYLES -->
	<!-- BEGIN PAGE LEVEL STYLES -->
	<link href="<?=BASE_PATH?>assets/css/pages/login-soft.css" rel="stylesheet" type="text/css"/>
...
.error {
	...
	<div class="content">
		<!-- BEGIN LOGIN FORM -->
		<form class="form-vertical login-form" action="<?=BASE_PATH?>login/actions.php" method="post">
        <input type="hidden" name="action" value="handle_login" />
        <input type="hidden" name="r" value="<?=$_GET['r']?>" />
			<h3 class="form-title">Login to your account</h3><div class="error"><?=$error?></div>
			...
			<div class="control-group">
				...
				<div class="controls">
					<div class="input-icon left">
						<i class="icon-user"></i>
						<input class="m-wrap placeholder-no-fix" type="text" autocomplete="off" placeholder="Username" name="username" value="<?=$username?>" />
					</div>
				</div>
			</div>
			<div class="control-group">
			...
			<div class="form-actions">
				...
				<input type="checkbox" name="remember_username" value="1" <?php if(isset($_COOKIE['username'])) {echo 'checked="checked"';}?> /> Remember me
				</label>
	...
	<!-- BEGIN COPYRIGHT -->
	<div class="copyright">
		2014 &copy; <?=COMPANY_NAME?> - All Rights Reserved.
	</div>
	<!-- END COPYRIGHT -->
	<!-- BEGIN JAVASCRIPTS(Load javascripts at bottom, this will reduce page load time) -->
	<!-- BEGIN CORE PLUGINS -->   <script src="<?=BASE_PATH?>assets/plugins/jquery-1.10.1.min.js" type="text/javascript"></script>
	<script src="<?=BASE_PATH?>assets/plugins/jquery-migrate-1.2.1.min.js" type="text/javascript"></script>
	<!-- IMPORTANT! Load jquery-ui-1.10.1.custom.min.js before bootstrap.min.js to fix bootstrap tooltip conflict with jquery ui tooltip -->
	<script src="<?=BASE_PATH?>assets/plugins/jquery-ui/jquery-ui-1.10.1.custom.min.js" type="text/javascript"></script>      
	...
	<script src="<?=BASE_PATH?>assets/plugins/jquery-validation/dist/jquery.validate.min.js" type="text/javascript"></script>
	<script src="<?=BASE_PATH?>assets/plugins/backstretch/jquery.backstretch.min.js" type="text/javascript"></script>
	<script type="text/javascript" src="<?=BASE_PATH?>assets/plugins/select2/select2.min.js"></script>
	<!-- END PAGE LEVEL PLUGINS -->
	<!-- BEGIN PAGE LEVEL SCRIPTS -->
	<script src="<?=BASE_PATH?>assets/scripts/app.js" type="text/javascript"></script>
	<script src="<?=BASE_PATH?>assets/scripts/login-soft.js" type="text/javascript"></script>      
